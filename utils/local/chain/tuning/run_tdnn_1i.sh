...
num_threads_extractor=3
num_processes_extractor=3
...
if ! cuda-compiled; then
  cat <<EOF && exit 1
This script is intended to be used with  s but you have not compiled Kaldi with CUDA
...
echo "stage 13 has started"
if [ $stage -le 13 ]; then
  # Get the alignments as lattices (gives the chain training more freedom).
  # use the same num-jobs as the alignments
  steps/align_fmllr_lats.sh --nj 10 --cmd "$train_cmd" ${lores_train_data_dir} \
    data/lang $gmm_dir $lat_dir
  rm $lat_dir/fsts.*.gz # save space
fi
echo "stage 14 has started"
...
if [ $stage -le 16 ]; then
  ...
  #modifed by pankaj, changed the folder from steps to steps_local
...
cp -R data/lang_chain/ data/lang_test_tgpr
cp -R data/lang_chain/ data/lang_test_bd_tgpr
cp -R data/lang_chain/ data/lang_test_bd_fgconst
cp -R data/lang_chain/ data/lang_test_tg

echo "stage 17 has started"
...
if [ $stage -le 17 ]; then
  ...
  utils/mkgraph.sh \
    ...
    $tree_dir $tree_dir/graph_bd_tgpr
fi
echo "stage 18 has started"
